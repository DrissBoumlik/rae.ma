
<div class="sl-container" id="sl-container">
    <!-- <div class="arrow hidden"></div> -->
    <ul class="list-group">
        <li class="list-group-item"><a href="https://api.whatsapp.com/send?phone=212662187074" target="_blank">
            <img src="https://projects.drissboumlik.com/rae.ma/images/social/Whatsapp.png" alt="">
        </a></li>
        <li class="list-group-item"><a href="https://facebook.com/messages/t/Etudierletranger.Ukraine.Russie" target="_blank">
            <img src="https://projects.drissboumlik.com/rae.ma/images/social/F-Messenger.png" alt="">
        </a></li>
        <li class="list-group-item"><a href="https://facebook.com/Etudierletranger.Ukraine.Russie/" target="_blank">
            <img src="https://projects.drissboumlik.com/rae.ma/images/social/facebook.png" alt="">
        </a></li>
    </ul>
    
</div>


<!-- #region SECTION: GO TOP BUTTON -->
<div class="up-container adds-container animated bounceOuDown" id="up-container">
    <a href="#top-page"><div class="up-btn adds-btn">
        <i class="fa fa-chevron-up"></i>
    </div></a>
</div>
<!-- #endregion -->

<div class="message-container hidden" id="message-container">
    <i class="far fa-times-circle"></i>
    <div class="container">
        <div class="message-body">
            <div class="row">
                <div class="col-sm-12 col-md-offset-0">
                    <p>ALLIANCE ÉDUCATIVE annonce sa participation au Forum International de l'Etudiant 2019. | Horaire : de 9h30 à 19h<br>
                    <!-- - <span class="_bounce_2 bg-light bold">Fés</span> (Marriott Hôtel Jnan Palace): jeudi 21 au samedi 23 Mars<br> -->
                    <!-- - <span class="_bounce_reverse_2 bg-light bold">Tanger</span> ( Centre Culturel Ahmed boukmakh ): jeudi 28 au samedi 30 Mars<br> -->
                    - <span class="_bounce_2 bg-light bold">Casablanca</span>( à la Foire Internationale de Casablanca ) : Jeudi 18 au Dimanche 21 Avril<br>
                    nous nous réjouissons si vous soyez venus si nombreux à cet important rendez-vous 😍😃.</p>
                </div>
                <!-- <div class="col-sm-2"><i class="far fa-smile-beam"></i></div> -->
            </div>
        </div>
    </div>
</div>

<?php 
    if(false)
        echo '
        <style>
            .sl-container.languages{
                right: unset;
                bottom: 10%;
                left: 50%;
                transform: translateX(-50%);    
            }
            .sl-container.languages .list-group-item img{
                height: 50px;
                width: 50px;
            }
            .sl-container.languages .list-group-item {
                display: inline-block;
                
            }

        </style>
        <div class="sl-container languages" id="sl-container-languages">
            <!-- <div class="arrow hidden"></div> -->
            <ul class="list-group">
                <li class="list-group-item"><a href="?lang=ar">
                    <img src="https://projects.drissboumlik.com/rae.ma/images/flags/ar.png" alt="">
                </a></li>
                <li class="list-group-item"><a href="?lang=fr">
                    <img src="https://projects.drissboumlik.com/rae.ma/images/flags/fr.png" alt="">
                </a></li>
                <li class="list-group-item"><a href="?lang=en">
                    <img src="https://projects.drissboumlik.com/rae.ma/images/flags/uk.png" alt="">
                </a></li>
            </ul>
            
        </div>';
